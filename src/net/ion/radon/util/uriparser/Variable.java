 ...
 * 
 * The variables can be typed by prefixing the variable name. Types are not required, if no type is specified, the variable is considered untyped.
 * 
 * Note: there is no predefined list of types as the handling of types is out of scope. The syntax simply allows variables to be associated with a type.
 * 
 ...
 * </pre>
 * 
 * @see <a href="http://bitworking.org/projects/URI-Templates/spec/draft-gregorio-uritemplate-03.html">URI Template (Internet Draft 3)</a>
...
public class Variable {
	 ...
	 *             If the specified name is an empty string.
	 */
	public Variable(String name, String def, VariableType type, Form form) throws NullPointerException, IllegalArgumentException {
		if (name == null)
			throw new NullPointerException("A variable must have a name, but was null");
		if (!isValidName(name))
			throw new IllegalArgumentException("The variable name is not valid: " + name);
	 ...
	 * 
	 * <p>
	 * Implementation note: this method replaces any character that could be interpreted as a regex meta-character, it is more efficient than using quotation (\Q...\E) for the whole string.
	 * 
