 ...
 * @see <a href="http://tools.ietf.org/html/rfc3986#appendix-A">RFC 3986 - Uniform Resource Identifier (URI): Generic Syntax - Appendix A. Collected ABNF for URI</a>
...
public class URICoder {
	 ...
	 * @throws UnsupportedEncodingException 
	 */
	public static String encode(String s, char c) throws UnsupportedEncodingException {
		if (s.length() == 0)
			return s;
	 ...
	 * @throws UnsupportedEncodingException 
	 */
	public static String minimalEncode(String s) throws UnsupportedEncodingException {
		if (s.length() == 0)
			return s;
		// Check whether we need to use UTF-8 encoder
		boolean ascii = isASCII(s);
	 ...
	 * @param e
	 *            A character that does not require encoding if found in the string.
	 */
	 ...
	 * @throws UnsupportedEncodingException 
	 */
	private static String encode_UTF8(String s, char e) throws UnsupportedEncodingException {
		boolean isStart = false, isEnd = false ;
		if (s.startsWith("" + e)) isStart = true ;
		if (s.endsWith("" + e)) isEnd = true ;
		
		String[] strs = StringUtil.split(s, e) ;
		List<String> estrs = ListUtil.newList() ;
		for (String string : strs) {
			estrs.add(URLEncoder.encode(string, "UTF-8")) ;
		}
		return (isStart ? e : "") +  StringUtil.join(estrs.toArray(), e) + (isEnd ? e : "");
...
//		return sb.toString();
	 ...
	 * @throws UnsupportedEncodingException 
	 */
	private static String minimalEncode_UTF8(String s) throws UnsupportedEncodingException {
