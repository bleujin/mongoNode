...
class ValueObjectAdaptor extends TypeAdaptor<Object> {
	...
	public Object read(TypeStrategy ts, Field field, ReadNode node) {
		if (field.getType() == Date.class) {
			return node.property(field.getName()).asDate();
		} else return node.property(field.getName()).asObject();
	}
...
class PrimitiveAdaptor extends TypeAdaptor {
	...
	public Object read(TypeStrategy ts, Field field, ReadNode node) {
		...
		if (field.getType() == int.class || field.getType() == Integer.class){
			return node.property(field.getName()).asInt();
		} else {
			return node.property(field.getName()).asObject();
		}
	}
